...
def server_layout(info):
    return html.Div(children=[
        ...
        html.H2("Informations Weather"),
        html.Div([
            html.P(f"Date: {info['date']}"),
            html.P(f"Heure: {info['time']}"),
            html.P(f"Ville: {info['weather']['city']}"),
            html.P(f"Température: {info['weather']['temperature']}"),
            html.P(f"Description: {info['weather']['description']}"),
        ], style={'marginBottom': 30, 'backgroundColor': '#F0F0F0', 'padding': '10px', 'borderRadius': '5px'}),
